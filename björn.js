...
const totalTrailMiles = (trails) => {
    return trails.reduce((c,n) => c + n.length, 0)
    // return sum(trail.length)
}

// Get the sum of all river miles
const totalRiverMiles = (rivers) => {
    return rivers.reduce((c,n) => c + n.length, 0)
    // return sum(rivers.length)
}


console.log(`
    B J Ö R N   T Ô R V O L D ' S   W I L D N E R N E S S   A D V E N T U R E S
    ***************************************************************************
